...
if [ -f "$role_dir/requirements.yml" ]; then
...
docker exec $container_id env TERM=xterm env ANSIBLE_FORCE_COLOR=1 ansible-playbook /etc/ansible/roles/role_under_test/$playbook
